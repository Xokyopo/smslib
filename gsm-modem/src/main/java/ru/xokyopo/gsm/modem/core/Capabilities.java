...
import ru.xokyopo.gsm.modem.entity.AbstractMessage;
import ru.xokyopo.gsm.modem.entity.OutboundBinaryMessage;
	...
	public boolean matches(OutboundMessage message) {
		...
		for (Caps c : Caps.values()) {
			if (c == Caps.CanSetSenderId) {
				...
				if (message.getEncoding() == AbstractMessage.Encoding.EncUcs2) bs.set(c.ordinal());
			} else if (c == Caps.CanSendBinaryMessage) {
				if ((message instanceof OutboundBinaryMessage) || (message.getEncoding() == AbstractMessage.Encoding.Enc8))
					bs.set(c.ordinal());
			} else if (c == Caps.CanSendUnicodeMessage) {
				if (message.getEncoding() == AbstractMessage.Encoding.EncUcs2) bs.set(c.ordinal());
